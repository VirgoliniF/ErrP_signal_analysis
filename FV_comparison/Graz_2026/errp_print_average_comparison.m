function [] = errp_print_average_comparison(all_data)
% errp_print_average_comparison
% 4 subplots: first 3 show EEG diff + velocity for each condition,
% last shows all EEG diffs overlaid (no velocity).
% Input: all_data - cell array of condition structs (output of main script)
    ...
    for idx_c = 1:n_conditions
        ...
        %condition_labels{idx_c} = sprintf('%s | %s', title_cond, cond.piero_analysis);
        ...
        diff_eegs{idx_c} = diff_eeg;
           ...
           % Print time of peak maximum negativity
        [~, idx_min] = min(diff_eeg);
        t_peak_neg   = t(idx_min);
        fprintf('[%s] Peak negativity: %.3f uV at t = %.3f s\n', ...
            title_cond, diff_eeg(idx_min), t_peak_neg);
        ...
        % ── Subplot idx_c: EEG diff + velocity for this condition ──
        subplot(4, 1, idx_c);
        hold on;
        grid on;
    ...
    % ── Subplot 4: all EEG diffs overlaid, no velocity ──
    subplot(4, 1, 4);
    hold on;
    grid on;
    ...
    filename = sprintf('comparison_%s_%s.png', ...
