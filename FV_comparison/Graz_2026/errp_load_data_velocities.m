function [T, Tu, E, Ek, Ck, Dk, eeg, eeg_un, setting, velz, twist, ePOS, eTYP] = errp_load_data_velocities(includepat,excludepat, setting)

    piero_analysis = setting.piero_analysis;
    ownership = setting.owner; % Francesca or Piero
    ...
    if strcmp(setting.data_owner, 'Francesca')
        datapath      = ['analysis/' artifactrej '/' spatialfilter '/bandpass/' num2str(length(eegchannels)) '/'];
        bagspath      = 'analysis/bags/aligned/';
    else
        datapath      = ['analysis_Piero/' artifactrej '/' spatialfilter '/bandpass/' num2str(length(eegchannels)) '/'];
        bagspath      = 'analysis_Piero/bags/aligned/';
    end
    ...
    % Le righe successive secondo Piero sono inutili. Sono d'accordo,
    % quindi le commento
    % if strcmp(ownership, 'Piero')
    %     eTYP = recomputeTYP(eTYP, setting.ev_mode);
    % 
    % elseif strcmp(ownership, 'Francesca')
    %     %eTYP = recomputeTYP_FV(eTYP, setting.ev_mode, setting.includepat);
    %     eTYP = recomputeTYP(eTYP, setting.ev_mode);     
    % 
    % else
    %     error('Wrong ownership assigned')
    % end


    if (setting.check_movement_cue == true && setting.remove_double_errors == true  )
        eTYP = recomputeTYP_mvt(eTYP);
    end
    ...
    % Extract events
    disp('     |- Extract events');
    evtidx = errp_util_get_event_type([CommandLx CommandRx ErrorLx ErrorRx NoReleaseFx NoReleaseRx NoReleaseLx MovErrLeft MovErrRight ErrLxMov ErrRxMov], eTYP);
    ...
    if strcmp(ownership, 'Piero')
        ...
        % Command correct
        Ck = false(ntrials, 1);
        Dk.crt_right = false(ntrials, 1);
        Dk.crt_left = false(ntrials, 1);
        correctindex = eTYP == CommandLx | eTYP == CommandRx;
        Ck(correctindex) = true;
        Dk.crt_right(eTYP == CommandRx) = true;
        Dk.crt_left(eTYP == CommandLx) = true;
        
        % Command error
        Ek = false(ntrials, 1);
        Dk.err_right = false(ntrials, 1);
        Dk.err_left = false(ntrials, 1);
        if strcmp(piero_analysis, 'visual')
            errorindex = eTYP == ErrorLx | eTYP == ErrorRx;
            Dk.err_right(eTYP == ErrorRx) = true;
            Dk.err_left(eTYP == ErrorLx) = true;
        elseif strcmp(piero_analysis, 'movement')
            errorindex = eTYP == ErrorLx | eTYP == ErrorRx;
            % errorindex = eTYP == ErrRxMov | eTYP == ErrLxMov | eTYP == MovErrRight | eTYP == MovErrLeft;
            %Dk.err_right(eTYP == ErrRxMov | eTYP == MovErrRight) = true;
            %Dk.err_left(eTYP == ErrLxMov | eTYP == MovErrLeft) = true;
        else
            error('Invalid piero_analysis parameter')
        end
        Ek(errorindex) = true;
    ...
    elseif strcmp(ownership, 'Francesca')
        ...
        if (strcmp(includepat, 'learn_errp_j3_1') || strcmp(includepat, 'learn_errp_j2_1') || strcmp(includepat, 'learn_errp_f2_2'))
            % Command correct
            correctindex = eTYP == CommandLx | eTYP == CommandRx | eTYP == ErrorLx | eTYP == ErrorRx; %Corretto: 361,363,5361,5363
            %correctindex = eTYP == CommandLx | eTYP == CommandRx | eTYP == ErrorLx | eTYP == ErrorRx;
            Ck(correctindex) = true;
            Dk.crt_right(eTYP == CommandRx | eTYP == ErrorRx) = true;
            Dk.crt_left(eTYP == CommandLx | eTYP == ErrorLx) = true;
        
            % Command error
            errorindex = eTYP == ErrLxMov | eTYP == ErrRxMov | eTYP == MovErrLeft | eTYP == MovErrRight; %Errore: 5461,5463,461,463
            %errorindex = eTYP == ErrLxMov | eTYP == ErrRxMov;
            Ek(errorindex) = true;
            Dk.err_right(eTYP == ErrRxMov | eTYP == MovErrRight)=true;
            Dk.err_left(eTYP == ErrLxMov | eTYP == MovErrLeft)=true;
            ...
            % Command correct
            correctindex = eTYP == CommandLx | eTYP == CommandRx | eTYP == ErrRxMov | eTYP == ErrLxMov;  %Corretto: 361,363,5461,5463
            %correctindex = eTYP == CommandLx | eTYP == CommandRx | eTYP == ErrRxMov | eTYP == ErrLxMov;
            Ck(correctindex) = true;
            Dk.crt_right(eTYP == CommandRx | eTYP == ErrRxMov)=true;
            Dk.crt_left(eTYP == CommandLx | eTYP == ErrLxMov)=true;
        
            % Command error
            errorindex = eTYP == ErrorLx | eTYP == ErrorRx | eTYP == MovErrLeft | eTYP == MovErrRight; %Errore: 5361,5363,461,463
            %errorindex = eTYP == ErrorLx | eTYP == ErrorRx;
            Ek(errorindex) = true;
            Dk.err_right(eTYP == ErrorRx | eTYP == MovErrRight)=true;
            Dk.err_left(eTYP == ErrorLx | eTYP == MovErrLeft)=true;

        end

    else
        error('Ownership not well setted')
    end 

    disp('Ownership:');
    disp(ownership);
    disp('Data ownership:');
    disp(setting.data_owner);
    disp('Piero Analysis:');
    disp(piero_analysis);
