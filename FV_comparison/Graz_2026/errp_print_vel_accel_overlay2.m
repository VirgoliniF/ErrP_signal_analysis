function [] = errp_print_vel_accel_overlay2(all_data)
...
%   all_data - cell array of condition structs (output of main script)
    ...
    condition_labels = {
        'Pipeline 1 - Channel FCz - Visual ErrP', ...
        'Pipeline 1 - Channel FCz - Movement ErrP - Vision allowed', ...
        'Pipeline 2 - Channel FCz - Movement ErrP - Blindfolded'
    };
    ...
    %% ── Figure 8: like Figure 6 but without velocity ─────────────────────
    fig8 = figure();
    fig8.Position = [100 100 1300 900];
    ...
    for idx_c = 1:n_conditions
        ...
        plot_accel_only(t, grand_mean, sem, col, condition_labels{idx_c}, vl_plot);
    end

    sgtitle(['channel: ' char(setting_ref.refchannel) ...
             ' | filter: ' setting_ref.spatialfilter ...
             ' | Mean \pm SEM  |  accel range-matched (no velocity)']);

    save_figure(fig8, setting_ref, 'SEM_accel_zero_range_matched_no_vel');
...
%                        (velocity [rad/s] and acceleration [rad/s²])
    ...
    if ~isempty(vl)
        ...
        elseif strcmp(accel_scale_mode, 'zero_tend_scaled')
            % Re-read y_lim to capture any forced ylim (e.g. [-4 4] on
            % subplot 2) that was set before calling this helper.
            y_lim  = ax_left.YLim;
            % Scale so the full acceleration range fits within ±10% of ylim,
            % i.e. accel is mapped to [-0.1*range, +0.1*range] symmetric
            % around zero — preserving relative peaks, no clamping.
            target_half = 0.1*(y_lim(2) - y_lim(1));  % e.g. 0.4 for [-4 4], 0.2 for [-2 2]
            a_half      = max(abs(accel(1:t(512*1))));                % max absolute value
            if a_half > 0
                a_scale = target_half / a_half;
            else
                a_scale = vel_scale;
            end
            a_scaled = accel * a_scale;
        end
        ...
        elseif strcmp(accel_scale_mode, 'zero_range_matched')
            ...
            ax_right = axes('Position', ax_left.Position, ...
                            ...
                            'YLim', right_lim, ...
                            'YColor', [0.2 0.2 0.2]);
            ylabel(ax_right, {'\Delta angular velocity [rad/s]', ' acceleration (range-matched)'});
            axes(ax_left);

        elseif strcmp(accel_scale_mode, 'zero_tend_scaled')
            % y_lim was already re-read in the scaling block above.
            right_lim = y_lim / vel_scale;
            ax_right = axes('Position', ax_left.Position, ...
                            'YAxisLocation', 'right', ...
                            'Color', 'none', ...
...
%% ── Helper: subplot like Figure 6 but without velocity line ─────────────
function plot_accel_only(t, grand_mean, spread, col, title_str, vl)
% Identical to plot_vel_accel with 'zero_range_matched' mode, but the
% velocity line is not drawn. The right axis keeps the same velocity scale
% so the acceleration (range-matched) is directly readable.
    ...
    if ~isempty(vl)
        ...
        % Same range-matching scale as Figure 6
        v_mean = nanmean(vl, 2);
        v_pp   = max(v_mean) - min(v_mean);
        a_pp   = max(accel)  - min(accel);
        if a_pp > 0
            a_scale = (v_pp / a_pp) * vel_scale;
        else
            a_scale = vel_scale;
        end
        a_scaled = accel * a_scale;

        % Dash-dot + low opacity (same style as Figure 6)
        h = plot(t_col, a_scaled, '-.', 'Color', [0.55 0.55 0.55], 'LineWidth', 1.2);
        h.Color(4) = 0.7;
        legend_entries{end+1} = 'acceleration (smoothed, range-matched)';

        % Right axis — same as Figure 6 (velocity scale)
        right_lim = y_lim / vel_scale;
        ax_right = axes('Position', ax_left.Position, ...
                        'YAxisLocation', 'right', ...
                        'Color', 'none', ...
