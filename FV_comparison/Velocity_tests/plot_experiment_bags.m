...
% =========================================================================
%  plot_experiment_bags.m
%
%  Adaptation of plot_grouped_by_velocity.m for CompVestibular / wheelchair
%  ErrP experiments.
%
%  WHAT THIS SCRIPT DOES
%  -------------------------------------------------------------------------
%  Reads all .bag files in a selected folder and groups trials ONLY by the
%  REAL wheelchair movement direction:
%
%       LEFT  = wheelchair rotated left  (angular.z > 0)
%       RIGHT = wheelchair rotated right (angular.z < 0)
%
%  IMPORTANT:
%    - cue correctness is ignored
%    - LEFT / ERR_LEFT / RIGHT / ERR_RIGHT are ignored
%    - MVT_CRT / MVT_ERR are ignored
%    - ONLY the REAL executed wheelchair movement matters
%
%  The grouping is therefore based on the sign of /gui/cmd_vel angular.z.
%
%  For every bag:
%    - all LEFT wheelchair movements are grouped together
%    - all RIGHT wheelchair movements are grouped together
%
%  OUTPUT:
%    Figure 1 -> Angular velocity profiles
%    Figure 2 -> Angular acceleration profiles
%
...
for b = 1:n_bags
    ...
    %  DETECT MOVEMENTS
    % ---------------------------------------------------------------------
    % We detect movement START and STOP directly from /gui/cmd_vel.
    %
    % This automatically groups trials by REAL wheelchair movement,
    % independently from:
    %   - cue
    %   - user correctness
    %   - ERR_LEFT / ERR_RIGHT
    ...
    for k = 1:numel(start_idx)
        ...
        %% Determine REAL movement direction
        ...
        for fn = fieldnames(odom)'
            ...
            mask = odom.(lbl).t >= tw0 & odom.(lbl).t <= tw1;
            ...
            step.([lbl '_t'])   = tt;
            step.([lbl '_wz'])  = ww;
            step.([lbl '_acc']) = safeDerive(tt, ww, SG_ORDER, SG_FRAMES);
        ...
        if ~isempty(enc_t)
            ...
            if sum(mask) > 4
                ...
                step.enc_acc = safeDerive(...
                    et(1:end-1), wz_e, SG_ORDER, SG_FRAMES);
    ...
    bag_data(end+1).steps = steps;
    bag_data(end).bagname = all_bags(b).name;
    bag_data(end).color   = BAG_COLORS(...
...
if isempty(bag_data)
    error('No valid bag data loaded');
end
...
for b = 1:numel(bag_data)

    ss = bag_data(b).steps;

    for k = 1:numel(ss)

        if strcmp(ss(k).direction, 'LEFT')
            LEFT_ALL{end+1} = {b, ss(k)};
        else
            RIGHT_ALL{end+1} = {b, ss(k)};
        end
...
fig1 = figure(...
    'Name', 'Wheelchair Angular Velocity', ...
    'NumberTitle', 'off', ...
    'Position', [100 100 1400 550]);

DIRS = {'LEFT', 'RIGHT'};
DATA = {LEFT_ALL, RIGHT_ALL};
...
sgtitle('Wheelchair Movements Grouped by REAL Movement Direction');
...
fig2 = figure(...
    'Name', 'Wheelchair Angular Acceleration', ...
    'NumberTitle', 'off', ...
...
sgtitle('Wheelchair Accelerations Grouped by REAL Movement Direction');
