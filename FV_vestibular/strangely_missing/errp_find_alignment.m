function alignment = errp_find_alignment(evtbag, evtgdf)
    ...
    events_of_interest = events_of_interest(events_of_interest ~= 1);
    ...
    %  Extract events from gdf structure
    evtgdf_index = get_event_index(events_of_interest, evtgdf.TYP);

    evtgdf.TYP = evtgdf.TYP(evtgdf_index);
    evtgdf.POS = evtgdf.POS(evtgdf_index);
    ...
    if(isempty(start_idx) == false)
        alignment = evtbag.POS(start_idx(1)) - evtgdf.POS(1);
    end
